...
class GenericFlatblocksTestCase(TestCase):
    def setUp(self):
        ...
        dummy_user = User.objects.create_user(
            u'johndoe', u'john@example.com', u'foobar'
        )
        ...
        self.admin_user = User.objects.create_superuser(
            u'admin', u'admin@example.com', u'foobar'
        )
    ...
    def parseTemplate(self, template_string, admin_user=False):
        ...
        c = Context(
            {'user': user, 'request': HttpRequest(user), 'LANGUAGE_CODE': 'en'}
        )
        return t.render(c)
    ...
    def testIntoArgument(self):
        ...
        '''
        self.assertRaises(
            (User.DoesNotExist, TemplateSyntaxError),
            self.parseTemplate,
            template_string,
        )
        settings.DEBUG = False
    ...
    def testContributedModels(self):
        ...
        self.assertTrue(
            isinstance(
                GenericFlatblock.objects.get(slug=u'title').content_object,
                models.Title,
            )
        )
        self.assertTrue(
            isinstance(
                GenericFlatblock.objects.get(slug=u'text').content_object,
                models.Text,
            )
        )
        self.assertTrue(
            isinstance(
                GenericFlatblock.objects.get(slug=u'image').content_object,
                models.Image,
            )
        )
        self.assertTrue(
            isinstance(
                GenericFlatblock.objects.get(
                    slug=u'title_and_text'
                ).content_object,
                models.TitleAndText,
            )
        )
        self.assertTrue(
            isinstance(
                GenericFlatblock.objects.get(
                    slug=u'title_text_and_image'
                ).content_object,
                models.TitleTextAndImage,
            )
        )

        # Test str method of contributed models
        self.assertTrue(
            GenericFlatblock.objects.get(slug=u'title').content_object.__str__()
        )
        self.assertTrue(
            GenericFlatblock.objects.get(slug=u'text').content_object.__str__()
        )
        self.assertTrue(
            GenericFlatblock.objects.get(slug=u'image').content_object.__str__()
        )
        self.assertTrue(
            GenericFlatblock.objects.get(
                slug=u'title_and_text'
            ).content_object.__str__()
        )
        self.assertTrue(
            GenericFlatblock.objects.get(
                slug=u'title_text_and_image'
